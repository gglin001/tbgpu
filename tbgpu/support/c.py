...
def _IOW(base, nr, typ):
  return functools.partial(_do_ioctl, 1, ord(base) if isinstance(base, str) else base, nr, typ)


def _IOR(base, nr, typ):
  return functools.partial(_do_ioctl, 2, ord(base) if isinstance(base, str) else base, nr, typ)


def _IOWR(base, nr, typ):
  return functools.partial(_do_ioctl, 3, ord(base) if isinstance(base, str) else base, nr, typ)
...
def pointer(x: T) -> POINTER[T]:
  return ctypes.pointer(x)  # type: ignore


if TYPE_CHECKING:
  _CFuncPtr = ctypes._CFunctionType
else:
...
class CFUNCTYPE(Generic[T, P], _CFuncPtr):
  ...
  def __class_getitem__(cls, key):
    return ctypes.CFUNCTYPE(key[0], *key[1])
...
class Array(Generic[T, U], ctypes.Array):
  ...
  def __class_getitem__(cls, key):
    return key[0] * get_args(key[1])[0]

  def __new__(cls, ty, l):
    return ty * l
...
class Struct(ctypes.Structure):
  ...
  def register_fields(cls, fields):
    ...
    for i, (name, *args) in enumerate(fields):
      setattr(cls, name, Field(*args, name=name, idx=i))
...
class Field:
  ...
  def __set_name__(self, owner, name):
    ...
    if hasattr(owner, "_real_fields_"):
      owner._real_fields_.append(entry)
    else:
      setattr(owner, "_real_fields_", [entry])
  ...
  def _resolve(self, cls):
    if self.bit_width:  # handle bitfields ourselves
      sl, set_mask = (
        slice(self.off, self.off + (sz := ceildiv(self.bit_width + self.bit_off, 8))),
        ~((mask := (1 << self.bit_width) - 1) << self.bit_off),
      )

      def b2i(obj):
        return int.from_bytes(memoryview(obj).cast("B")[sl], sys.byteorder)

      def bset(obj, v):
        memoryview(obj).cast("B")[sl] = ((b2i(obj) & set_mask) | v << self.bit_off).to_bytes(sz, sys.byteorder)
    ...
    # pull the CField descriptor from a dummy class, zero length arrays are so ctypes manages references to child objects for us
      ...
      cf = type(
        ...
        (ctypes.Structure,),
        {
          "_layout_": "ms",
          "_pack_": 1,
          "_fields_": [(str(i), ctypes.c_byte * 0) for i in range(self.idx)] + [("_", ctypes.c_byte * self.off), ("v", self.typ)],
        },
      ).v  # type: ignore
  ...
  def __get__(self, obj, objtype=None):
    return self._resolve(objtype).__get__(obj, objtype) if objtype else self

  def __set__(self, obj, value):
    self._resolve(obj.__class__).__set__(obj, value)
...
def init_c_var(ty, creat_cb):
  return (creat_cb(v := ty()), v)[1]
...
class DLL(ctypes.CDLL):
  ...
  def findlib(nm: str, paths: list[str], extra_paths=[]):
    ...
    for p in paths:
      libpaths = {
        "posix": ["/usr/lib64", "/usr/lib", "/usr/local/lib"],
      ...
      for pre in (pathlib.Path(pre) for pre in ([path] if path else []) + libpaths.get(os.name, []) + libpaths.get(sys.platform, []) + extra_paths):
        ...
        if WIN or OSX:
          for base in [f"lib{p}.dylib", f"{p}.dylib", str(p)] if OSX else [f"{p}.dll"]:
            if (l := pre / base).is_file() or (OSX and "framework" in str(l) and l.is_symlink()):
              return str(l)
        else:
  ...
  def __init__(self, nm: str, paths: str | list[str], extra_paths=[], emsg="", **kwargs):
    ...
    if path := DLL.findlib(nm, paths if isinstance(paths, list) else [paths], extra_paths if isinstance(extra_paths, list) else [extra_paths]):
      ...
      try:
        ...
        self.emsg = str(e)
        if DEBUG >= 3:
          print(f"loading {nm} failed: {e}")
    elif DEBUG >= 3:
      print(f"loading {nm} failed: not found on system")
  ...
  def bind(self, restype, *argtypes):
    def wrap(fn):
      ...
      def wrapper(*args):
        nonlocal cfunc
        if cfunc is None:
          (cfunc := getattr(self, fn.__name__)).argtypes, cfunc.restype = argtypes, restype
        return cfunc(*args)
