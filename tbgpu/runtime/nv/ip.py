...
class NVRpcQueue:
  ...
  def _send_rpc_record(self, func: int, msg: bytes):
    ...
    if first < len(msg):
      self.queue_mv[: len(msg) - first] = msg[first:]
  ...
  def wait_resp(self, cmd: int, timeout: int = 10000) -> bytes:
    ...
    while (int(time.perf_counter() * 1000) - start_time) < timeout:
      if (msg := next((message for func, message in self.read_resp() if func == cmd), None)) is not None:
        return msg
  ...
  def prep_ucode(self):
    ...
    while True:
      ...
      match vbios_bytes[vbios_off + pci_blck + nv.OFFSETOF_PCI_DATA_STRUCT_CODE_TYPE]:
        case nv.NV_BCRT_HASH_INFO_BASE_CODE_TYPE_VBIOS_BASE:
          block_size = imglen
...
class NV_FLCN_COT(NV_IP):
  ...
  def init_hw(self):
    ...
    boot_args = nv.GSP_ACR_BOOT_GSP_RM_PARAMS(
...
class NV_GSP(NV_IP):
  ...
  def init_rm_args(self, queue_size=0x40000):
    ...
    self.cmd_q_view[: ctypes.sizeof(nv.msgqTxHeader)] = bytes(
      nv.msgqTxHeader(
        version=0,
        size=queue_size,
        entryOff=0x1000,
  ...
  def init_libos_args(self):
    ...
    libos_structs = [
      nv.LibosMemoryRegionInitArgument(
  ...
  def rpc_alloc_memory(self, hDevice: int, hClass: int, paddrs: list[tuple[int, int]], length: int, flags: int, client: int | None = None) -> int:
    ...
    rpc = nv.rpc_alloc_memory_v(
      hClient=(client := client or self.priv_root),
      hDevice=hDevice,
      hMemory=(handle := next(self.handle_gen)),
      hClass=hClass,
      flags=flags,
      pteAdjust=0,
      format=6,
      length=length,
      pageCount=len(paddrs),
    )
  ...
  def rpc_rm_alloc(self, hParent: int, hClass: int, params: Any, client=None) -> int:
    ...
    if hClass == nv_gpu.NV01_DEVICE_0 and client != self.priv_root:
      self.device = obj  # save user device handle
  ...
  def rpc_rm_control(self, hObject: int, cmd: int, params: Any, client=None, extra=None):
    if cmd == nv_gpu.NVB0CC_CTRL_CMD_POWER_REQUEST_FEATURES:
      self.rpc_rm_control(
        hObject,
        nv_gpu.NVB0CC_CTRL_CMD_INTERNAL_PERMISSIONS_INIT,
        nv_gpu.NVB0CC_CTRL_INTERNAL_PERMISSIONS_INIT_PARAMS(
          bAdminProfilingPermitted=1,
          bDevProfilingPermitted=1,
          bCtxProfilingPermitted=1,
          bVideoMemoryProfilingPermitted=1,
          bSysMemoryProfilingPermitted=1,
        ),
        client=client,
      )
    elif cmd == nv_gpu.NVB0CC_CTRL_CMD_ALLOC_PMA_STREAM:
      params.hMemPmaBuffer = self.rpc_alloc_memory(
        self.device,
        nv_gpu.NV01_MEMORY_LIST_SYSTEM,
        extra[0].meta.mapping.paddrs,
        extra[0].size,
        pma_flags := (nv_gpu.NVOS02_FLAGS_PHYSICALITY_NONCONTIGUOUS << 4 | nv_gpu.NVOS02_FLAGS_MAPPING_NO_MAP << 30),
        client=client,
      )
      params.hMemPmaBytesAvailable = self.rpc_alloc_memory(
        self.device,
        nv_gpu.NV01_MEMORY_LIST_SYSTEM,
        extra[1].meta.mapping.paddrs,
        extra[1].size,
        pma_flags | nv_gpu.NVOS02_FLAGS_ALLOC_USER_READ_ONLY_YES << 21,
        client=client,
      )
  ...
  def rpc_set_gsp_system_info(self):
    def bdf_as_int(s):
      return 0x000 if s.startswith("usb") or s.startswith("remote") else (int(s[5:7], 16) << 8) | (int(s[8:10], 16) << 3) | int(s[-1], 16)

    pcidev = self.nvdev.pci_dev
    data = nv.GspSystemInfo(
      gpuPhysAddr=pcidev.bar_info(0)[0],
      gpuPhysFbAddr=pcidev.bar_info(1)[0],
