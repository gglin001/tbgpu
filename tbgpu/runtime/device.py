...
from tbgpu.runtime.nv.nvdev import NVDev, NVMemoryManager
from tbgpu.runtime.transport import APLRemotePCIDevice, System


def nv_flags(reg, **kwargs):
  return ctypes.c_uint32(sum(((getattr(nv_gpu, f"{reg}_{key}_{val}".upper()) if isinstance(val, str) else val) <<
                              getattr(nv_gpu, f"{reg}_{key}".upper())[1]) for key, val in kwargs.items())).value
  ...
  def __init__(self, dev: "TBGPUDevice", view: MMIOInterface | None = None, **kwargs):
    ...
    if prefix not in QMD.fields:
      QMD.fields[prefix] = {
        ...
        **{name[len(prefix) + 1:] + f"_{idx}": value(idx) for name, value in nv_gpu.__dict__.items() for idx in range(8)
    ...
    if self.active_qmd is not None:
      for index in range(2):
        if self.active_qmd.read(f"release{index}_enable") == 0:
          ...
          addr_off = self.active_qmd.field_offset(f"release{index}_address_lower" if self.active_qmd.ver < 4 else f"release_semaphore{index}_addr_lower")
          ...
          val_off = self.active_qmd.field_offset(f"release{index}_payload_lower" if self.active_qmd.ver < 4 else f"release_semaphore{index}_payload_lower")
...
class TBGPUAllocator:
  ...
  def _copyin(self, dest: Buffer, src: memoryview):
    for off in range(0, src.nbytes, self.staging[0].size):
      ...
      NVCopyQueue().wait(self.dev.timeline_signal, self.dev.timeline_value - 1).copy(dest.offset(off, chunk), stage.offset(0, chunk), chunk) \
  ...
  def _copyout(self, dest: memoryview, src: Buffer):
    for off in range(0, dest.nbytes, self.staging[0].size):
      ...
      NVCopyQueue().wait(self.dev.timeline_signal, self.dev.timeline_value - 1).copy(stage.offset(0, chunk), src.offset(off, chunk), chunk) \
                   .signal(self.dev.timeline_signal, self.dev.next_timeline()).submit(self.dev)
      self.staging_timeline[self.staging_idx] = self.dev.timeline_value - 1
  ...
  def _transfer(self, dest: Buffer, src: Buffer, size: int, src_dev: "TBGPUDevice", dest_dev: "TBGPUDevice"):
    ...
    NVCopyQueue().wait(src_dev.timeline_signal, src_dev.timeline_value - 1).copy(dest, src, size) \
...
class TBGPUDevice:
  def __init__(self, ordinal: int = 0):
    ...
    self.channel_group = self.iface.rm_alloc(self.nvdevice, nv_gpu.KEPLER_CHANNEL_GROUP_A,
                                             nv_gpu.NV_CHANNEL_GROUP_ALLOCATION_PARAMETERS(engineType=nv_gpu.NV2080_ENGINE_TYPE_GRAPHICS))
    self.gpfifo_area = self.iface.alloc(0x300000, contiguous=True, cpu_access=True, force_devmem=True,
                                        uncached=False)
    ctxshare = self.iface.rm_alloc(self.channel_group, nv_gpu.FERMI_CONTEXT_SHARE_A,
                                   nv_gpu.NV_CTXSHARE_ALLOCATION_PARAMETERS(hVASpace=vaspace,
  ...
  def _new_gpu_fifo(self, gpfifo_area, ctxshare, channel_group, offset=0, entries=0x400, compute=False, video=False):
    ...
    if compute:
      ...
      self.debugger = self.iface.rm_alloc(self.nvdevice, nv_gpu.GT200_DEBUGGER,
                                          nv_gpu.NV83DE_ALLOC_PARAMETERS(hAppClient=self.iface.root, hClass3dObject=self.debug_compute_obj))
    else:
  ...
  def _query_gpu_info(self, *reqs):
    nvrs = [getattr(nv_gpu, "NV2080_CTRL_GR_INFO_INDEX_" + req.upper(), getattr(nv_gpu, "NV2080_CTRL_GR_INFO_INDEX_LITTER_" + req.upper(), None))
            for req in reqs]
    if self.is_nvd():
      info = self.iface.rm_control(self.subdevice, nv_gpu.NV2080_CTRL_CMD_INTERNAL_STATIC_KGR_GET_INFO,
  ...
  def _setup_gpfifos(self):
    ...
    NVComputeQueue().setup(compute_class=self.iface.compute_class, local_mem_window=self.local_mem_window,
                           shared_mem_window=self.shared_mem_window) \
                    .signal(self.timeline_signal, self.next_timeline()).submit(self)
    NVCopyQueue().wait(self.timeline_signal, self.timeline_value - 1).setup(copy_class=self.iface.dma_class) \
                 .signal(self.timeline_signal, self.next_timeline()).submit(self)
    self.synchronize()
  ...
  def _ensure_has_local_memory(self, required):
    ...
    NVComputeQueue().wait(self.timeline_signal, self.timeline_value - 1).setup(local_mem=self.shader_local_mem.va_addr,
