...
__device__ __forceinline__ float warp_reduce_sum_qkv(float value) {
  for (int offset = WARP_SIZE / 2; offset > 0; offset >>= 1) value += __shfl_down_sync(0xffffffffu, value, offset);
  return value;
}

extern "C" __global__ void flash_attn_qkv_fused_fwd(
  ...
  for (unsigned int kv_start = 0; kv_start < seq_len; kv_start += BLOCK_N) {
    ...
    if (active_row) {
      for (unsigned int key = 0; key < valid_k; ++key) {
        ...
        float score = warp_reduce_sum_qkv(partial) * softmax_scale;
        if (lane == 0) {
  ...
  const uint64_t out_row_base = ((uint64_t)batch_idx * seq_len + query_idx) * channels + head_idx * head_dim;
  for (unsigned int frag = 0; frag < frag_count; ++frag) {
    const unsigned int dim = lane + frag * WARP_SIZE;
    if (dim < head_dim) o[out_row_base + dim] = acc_frag[frag] * inv_row_l;
  }
}
